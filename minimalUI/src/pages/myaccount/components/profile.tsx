...
import { Sparkles, Phone, Mail, User, Building, } from 'lucide-react';
import { Edit, } from "@mui/icons-material";
import { motion } from 'framer-motion';

const MyAccountProfile = ({ handleSubmit, handleChange, userData, isEditingProfile, setIsEditingProfile }: any) => {
    const theme = useTheme();


    const profileItems = [
        { label: 'First Name', name: 'firstName', icon: User, color: '#667eea' },
        { label: 'Last Name', name: 'lastName', icon: User, color: '#764ba2' },
        { label: 'Email', name: 'email', icon: Mail, color: '#f093fb' },
        { label: 'Phone Number', name: 'phone', icon: Phone, color: '#f5576c' },
        { label: 'Status', name: 'status', icon: Sparkles, color: '#4ecdc4' },
        { label: 'Role', name: 'role', icon: Building, color: '#45b7d1' }
    ];
                ...
                <Stack spacing={6}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2}>
                        ...
                        <Stack direction="row" gap={3}>
                            ...
                            <Button startIcon={<Edit />} onClick={() => setIsEditingProfile(!isEditingProfile)}
                                sx={{
                        ...
                        {profileItems.map((item, index) => (
                            <Grid size={{ xs: 12, sm: 6, md: 4 }} key={index}>
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} >
                                    <Box sx={{
                                        p: 3, borderRadius: 3, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', transition: 'all 0.3s ease',
                                        '&:hover': { background: 'rgba(255,255,255,0.1)', transform: 'translateY(-4px)', boxShadow: '0 15px 30px rgba(0,0,0,0.3)' }
                                    }}>
                                        <Stack direction="row" alignItems="center" spacing={2}>
                                            <Box sx={{
                                                width: 45, height: 45, borderRadius: 2, background: `linear-gradient(135deg, ${item.color}20, ${item.color}10)`,
                                            ...
                                            <Box sx={{ width: '100%' }}>
                                                ...
                                                <TextField fullWidth placeholder={item.label} name={item.name}
                                                    value={userData[item.name as keyof typeof userData] || ""} onChange={handleChange} disabled={!isEditingProfile}
